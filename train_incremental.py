...
def training(dataset, opt, pipe, cfg_kitti, cfg_box, cfg_sd, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint_dir, debug_from, dyn_obj_list=['car'], exp_note='', run=None, args=None, output_dir=None):
    ...
    #获取全部视角并处理
    unique_viewpoint = scene.getTrainCameras()
    current_viewpoint_stack = [unique_viewpoint[0]]
    future_viewpoint_stack = unique_viewpoint[1:]
    render_viewpoint_stack = current_viewpoint_stack + future_viewpoint_stack[:5]
    name = [i.image_name for i in render_viewpoint_stack]
    print(name)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # 每200轮新增一帧进行训练
        if (iteration>200) and (iteration % 200 == 1):
            frame_iter = 100
            render_viewpoint_stack = future_viewpoint_stack.copy()[:6]
            current_viewpoint_stack.append(future_viewpoint_stack.pop(0))


        # Pick a random Camera
        if not viewpoint_stack:
            viewpoint_stack = current_viewpoint_stack.copy()
        
        #选取当前帧进行优化frame_iter轮
        if frame_iter:
        ...
        with torch.no_grad():
            if not args.no_wandb:
                ...
                # 每两百轮保存一次render结果，testing_iterations存有保存轮次，render_viewpoint_stack作为参数传过去
                training_report(iteration, testing_iterations, scene, all_bboxes, gaussians, pipe, background, dyn_obj_list, cfg_sd=cfg_sd, viewpoint_stack=render_viewpoint_stack, scalar_kwargs=scalar_kwargs, save_dir=save_dir)
...
def training_report(iteration, testing_iterations, scene : Scene, all_bboxes, gaussians, pipe, background, dyn_obj_list, cfg_sd=None, viewpoint_stack=None, scalar_kwargs=None, save_dir=None):
    ...
    if iteration in testing_iterations:
        ...
        if viewpoint_stack is not None:
            config = {'name': 'test', 'cameras' : viewpoint_stack}
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            # 为了速度，我去掉了渲染其他视角图片的过程，需要的话可以参照原版加回来。
...
if __name__ == "__main__":
    ...
    # 修改了需要test的轮数
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[i*200 for i in range(1, 500)])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[57200])
